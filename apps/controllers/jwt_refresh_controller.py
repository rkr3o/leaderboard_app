...
class JWTRefreshController:
    ...
    def validate_and_refresh(self):
        try:
            payload = jwt.decode(self.refresh_token, JWT_SECRET, algorithms=[JWT_ALGORITHM])
        ...
        token_record = JWTToken.objects.filter(token=self.refresh_token, is_active=True).first()
        if not token_record:
